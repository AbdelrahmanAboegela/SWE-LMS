    ...
    public partial class frmBorrower : Form
    {
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txt_bid.Text) ||
                ...
                string.IsNullOrEmpty(txt_mname.Text) ||
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            {
                // Get the BorrowerId from the selected row in the DataGridView
                string borrowerId = dtg_ABorrowLists.CurrentRow.Cells[0].Value.ToString();

                // Delete the borrower from the database
                sql = "DELETE FROM `tblborrower` WHERE `BorrowerId` = '" + borrowerId + "'";
                config.Execute_Query(sql);

                MessageBox.Show("Borrower has been deleted from the database.", "Success", MessageBoxButtons.OK);
                btn_New_Click(sender, e);
            }
        }
